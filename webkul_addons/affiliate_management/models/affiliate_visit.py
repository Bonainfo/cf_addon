...
class AffiliateVisit(models.Model):
    ...
    def _calc_type_name(self):
        for record in self:
            if record.affiliate_type == 'product':
                ...
                _logger.info('-----type product-name-%r---',record.type_name)
            if record.affiliate_type == 'category':
                record.type_name = record.env['product.public.category'].browse([record.type_id]).name
                _logger.info('-----type category-name-%r---',record.type_name)


    affiliate_method = fields.Selection([("ppc","Pay Per Click"),("pps","Pay Per Sale")],string="Order Report",readonly='True',states={'draft': [('readonly', False)]},help="state of traffic ether ppc or pps")
    ...
    unit_price = fields.Float(string="Product Unit Price", related='sales_order_line_id.price_unit',
    ...
    sale_refernce = fields.Many2one('sale.order',related='sales_order_line_id.order_id',string='Order Reference')
...
# button on view action
    def action_confirm(self):
        _logger.info("--------self.affiliate_method--%r----",self.affiliate_method)
        check_enable_ppc = self.env['ir.values'].get_default('affiliate.config.setting', 'enable_ppc')
        _logger.info("--------check_enable_ppc--%r----",check_enable_ppc)
        ...
        _logger.info('===============commission status====commission status ==== %r==',status)
...
# scheduler according to the scheduler define in data automated scheduler
    ...
    def process_scheduler_queue(self):
        ...
        payment_day = self.env['ir.values'].get_default('affiliate.config.setting', 'payment_day')
        threshold_amt = self.env['ir.values'].get_default('affiliate.config.setting', 'minimum_amt')
            ...
            if visits:
                ...
                if total_comm_per_user >= threshold_amt and payment_date:
                    ...
                    dic={
                                ...
                                'product_id':self.env['ir.values'].get_default('affiliate.config.setting', 'aff_product_id'),
                            }
                    line = self.with_context({'journal_id':inv_id.journal_id.id}).env['account.invoice.line'].create(dic)
                    for v in visits:
                        v.state = 'invoice'
                        v.act_invoice_id = inv_id.id
        _logger.info("==================Scheduler Runs for year=========================")
    ...
    def create_invoice(self):
        _logger.info('----server action  self ids ids--%r--------',self._context.get('active_ids'))
                    ...
                    if self.affiliate_program_id.pps_type == 's':
                        ...
                        if self.affiliate_program_id.pps_type == 'a' and product_exists:#for pps_type advance
                            ...
                            if commission and commission_type:
                                ...
                                response={
                                    'is_error':1,
                                    # 'message':'In Advance commision or commission_type is absent',
    ...
    def advance_pps_type_calc(self):
        _logger.info("------------in the method of advance pps calulation---------------")
    ...
    def process_ppc_maturity_scheduler_queue(self):
        # _logger.info("-----------ppc-------- mturity-------Scheduler-----------------")
        check_enable_ppc = self.env['ir.values'].get_default('affiliate.config.setting', 'enable_ppc')
        all_ppc_visits = self.search([('affiliate_method','=','ppc'),('state','=','draft')])
        # _logger.info("-----------all_ppc_visits--%r--------------",all_ppc_visits)
        # _logger.info("-----------check_enable_ppc--%r--------------",check_enable_ppc)
