...
class website_affiliate(Home):
    ...
    def affiliate(self, **kw):
        ...
        enable_forget_pwd = request.env['ir.values'].get_default('affiliate.config.setting', 'enable_forget_pwd')
        enable_login = request.env['ir.values'].get_default('affiliate.config.setting', 'enable_login')
        enable_signup = request.env['ir.values'].get_default('affiliate.config.setting', 'enable_signup')
        how_it_works = request.env['affiliate.config.setting'].sudo().get_default_how_it_works_values()
    ...
    def register(self, **kw):
        _logger.info('=================sign up page==============')
        token = request.httprequest.args.get('token')
        user = request.env['affiliate.request'].sudo().search([('signup_token','=',token)])
        term_condition = request.env['ir.values'].get_default('affiliate.config.setting', 'term_condition')
        values = {}
    ...
    def register_affiliate(self, **kw):
        ...
        if aff_request and kw.get('confirm_password') == kw.get('password') and aff_request.signup_token == kw.get('token'):
            template_user_id = literal_eval(request.env['ir.config_parameter'].get_param('auth_signup.template_user_id', 'False'))
            template_user = request.env['res.users'].sudo().browse(template_user_id)
            auto_approve_request = request.env['ir.values'].get_default('affiliate.config.setting', 'auto_approve_request')
    ...
    def report(self, **kw):
        ...
        currency_id = request.env.user.company_id.currency_id
        visits = request.env['affiliate.visit']
        ppc_visit = visits.search([('affiliate_method','=','ppc'),('affiliate_partner_id','=',partner.id),'|','|',('state','=','invoice'),('state','=','confirm'),('state','=','paid')])
        pps_visit = visits.search([('affiliate_method','=','pps'),('affiliate_partner_id','=',partner.id),'|','|',('state','=','invoice'),('state','=','confirm'),('state','=','paid')])
        values = {
        'pending_amt':partner.pending_amt,
        'approved_amt':partner.approved_amt,
    ...
    def traffic(self, page=1, date_begin=None, date_end=None, **kw):
        ...
        partner = request.env.user.partner_id
        visits = request.env['affiliate.visit']
        domain = [('affiliate_partner_id','=',partner.id),('affiliate_method','=','ppc'),'|','|',('state','=','invoice'),('state','=','confirm'),('state','=','paid')]
    ...
    def aff_traffic_form(self, traffic=None, **kw):
        traffic_visit = request.env['affiliate.visit'].browse([traffic])
    ...
    def aff_order(self, page=1, date_begin=None, date_end=None, **kw):
        ...
        partner = request.env.user.partner_id
        visits = request.env['affiliate.visit']
        domain = [('affiliate_partner_id','=',partner.id),('affiliate_method','=','pps'),'|','|',('state','=','invoice'),('state','=','confirm'),('state','=','paid')]
        if date_begin and date_end:
            domain += [('create_date', '>', date_begin), ('create_date', '<=', date_end)]
        traffic_count = visits.search_count(domain)
        pager = request.website.pager(
    ...
    def aff_order_form(self, order=None, **kw):
        order_visit = request.env['affiliate.visit'].browse([order])
