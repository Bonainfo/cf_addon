...
def _getProductData(p_data, base_url, currency_symbol, currency_position, lang_obj):
    ...
    for prod in p_data:
        result.append({
            ...
            "discount"      :prod['pharmacy_discount'] or 0,
            "max_bonus"     :prod['max_percentage_bonus'] or 0,
            "seller_name"   :prod['marketplace_seller_id'] and prod['marketplace_seller_id'][1],

            })
    return result

def _get_product_fields():
    return ['name','product_variant_id','product_variant_count','price','description_sale','lst_price','website_price','pharmacy_discount','max_percentage_bonus','marketplace_seller_id']
...
class Mobikul(models.Model):
    ...
    def check_mobikul_addons(self):
        ...
        result['wishlist'] = ir_model_obj.search([('state', '=', 'installed'),('name', '=', 'website_wishlist')]) and True or False
        ...
        result['marketplace_pharmacist_details'] = ir_model_obj.search([('state', '=', 'installed'),('name', '=', 'marketplace_pharmacist_details')]) and True or False
        return result

    def email_verification_defaults(self):
        ir_values = self.env['ir.values'].sudo()
        send_email_on_signup = ir_values.get_default('email.verification.config', 'send_email_on_signup')
        # token_validity = ir_values.get_default('email.verification.config', 'token_validity')
        restrict_unverified_users = ir_values.get_default('email.verification.config', 'restrict_unverified_users')
        return {
    ...
    def review_defaults(self):
        ir_values = self.env['ir.values'].sudo()
        review_no = ir_values.get_default('website.review.config', 'review_no') or 1
        auto_publish = self.env['ir.values'].get_default('website.review.config', 'auto_publish')
        message_when_unpublish = self.env['ir.values'].get_default(
            'website.review.config', 'message_when_unpublish') or _("Congratulation!!! your review has been submitted successfully, it will publish with in 24 hours.")
        return {'review_no': review_no, 'auto_publish': auto_publish, 'message_when_unpublish': message_when_unpublish}
    ...
    def resetPassword(self, login):
        ...
        try:
            ...
            response['message'] = _("Mail Server is under maintenance.")
        except Exception,e:
        ...
        try:
            with self.env.cr.savepoint():
                if self.check_mobikul_addons().get('odoo_marketplace') and data.get('is_seller'):
                    ...
                    if data.get('isSocialLogin') :
                        res = self._socialSignUp( no_invitation_mail, template_user , values, data)
                    else:
                        res = self._simpleSignUp( no_invitation_mail, template_user , values, data)

                    if res.get('status'):
                        return [True, res.get('user').id, res.get('user').partner_id.id, res.get('msg')]
                    else:
    ...
    def _socialSignUp(self,no_invitation_mail, template_user , values, data):
        response = {"status": False, "user" : False, "msg" :""}
        if data.get('isSocialLogin') and data.get('authUserId') and data.get('authProvider'):
            user = template_user.with_context(no_reset_password=no_invitation_mail).copy(values)
            response = {"status": True, "user" : user, "msg" :_("Created successfully. ")}
        else:
            response = {"status": False, "user" : False, "msg" :_("All Fields are mandatory in Social SignUp.('authUserId','authProvider','login')")}
        return response



    def _simpleSignUp(self,no_invitation_mail, template_user , values, data):
        response = {"status": False, "user" : False, "msg" :""}
        if data.get('job_title') and data.get('phone') and data.get('pharmacy_name'):
            user = template_user.with_context(no_reset_password=no_invitation_mail).copy(values)
            user.partner_id.function = data.get("job_title")
            user.partner_id.mobile = data.get("phone")
            ...
            pharmacy_partner = self.env['res.partner'].sudo().create(vals)
            user.partner_id.parent_id = pharmacy_partner.id
            response = {"status": True, "user" : user, "msg" :_("Created successfully. ")}

        else:
            response = {"status": False, "user" : False, "msg" :_("All Fields are mandatory.('pharmacy_name','job_title','phone')")}
        return response
    ...
    def _default_language(self):
        lc = self.env['ir.values'].get_default('res.partner', 'lang')
    ...
    country_ids = fields.Many2many('res.country','mobikul_coumtry_rel','mobikul_id','country_id', string='Countries')
...
class MobikulPushNotificationTemplate(models.Model):
    ...
    notification_title = fields.Char('Title', translate=True)
    ...
    condition = fields.Selection([
        ...
        ], string='Condition', required=True, default='signup',help="Send push Notification on new customer SignUp, on Place Order and on Order State.")
    state_cond = fields.Selection([
        ('ordercancel',"On Order Cancellation"),
        ('orderdeliver',"On Confirm Delivery")
        ], string='Order State Condition', required=True, default='ordercancel',help="Send push Notification on Order State (Cancel and Confirm Delivery).")
