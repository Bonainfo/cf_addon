odoo.define('website_ajax_login_pharmacistplace.website_ajax_login_pharmacistplace', function(require) {
    ...
    $(document).ready(function() {
                        ...
                        .then(function(response) {
                            $.each(response, function(key, value) {
                                ...
                                if (value.validation_endpoint.search("googleapis") > 0) {
                                    link_provider('btn-google', value.auth_link);
                                }
        ...
        function custom_msg(element_id, status, msg) {
            if (status == true) {
                element_id.empty().append("<div class='alert alert-danger col-md-12 text-center' id='Wk_err' style='padding:8px;'>" + msg + "<button type='button' class='close' data-dismiss='alert' aria-label='Close'><span class='res fa fa-times ' aria-hidden='true'></span></button></div>");
            }
            if (status == false) {
                element_id.empty();
            }
        ...
        $('#linklogin2_from_signup').on('click', function() {
            $('#wk_Modal_reset').modal('hide');
            $('#wk_Modal_signup').modal('hide');
            $('#wk_Modal_login').appendTo('body').modal('show');
        });

        $('#linksignup, a[href*="/web/signup"]').on('click', function() {
            $("<div id='wk_ajax_loader'/>").appendTo('body');
            if (wk_block_ui)
            {
        ...
        // code added for click on signup menu
        $('body').on("click",'a[href*="/web/signup"]', function(ev) {
            ev.preventDefault()
            $('#wk_Modal_reset').modal('hide');
            $('#wk_Modal_login').modal('hide');
            $('#wk_Modal_signup').appendTo('body').modal('show');
        ...
        $('#wk_signup_button').on('click', function() {
            ...
            var signup_phone = $(this).parent().parent().parent().parent().find('#signup_phone');
            var signup_job_title = $(this).parent().parent().parent().parent().find('#signup_job_title');
            var signup_pharmacy_name = $(this).parent().parent().parent().parent().find('#signup_pharmacy_name');
            var t_n_c = $(this).parent().parent().parent().parent().find('#terms_n_cond_checkbox');
            var t_n_c_checked = true
            if ($(t_n_c).prop("checked") == false){
                t_n_c_checked = false
            }
            ...
            var input = {
                ...
                'db': 'test24',
                'name': name.val(),
                'redirect': '/page/selectyourplan',
                'phone': signup_phone.val(),
                'job_title': signup_job_title.val(),
                'pharmacy_name': signup_pharmacy_name.val(),
                't_n_c_checked':t_n_c_checked,
            };
            ...
            ajax.jsonRpc('/website_ajax_login/signup', 'call', input)
                .then(function(res) {
                    ...
                    if (typeof(res['uid']) != "undefined") {
                        ...
                        if (list_res.indexOf("t_n_c") > 0) {
                            custom_msg($('#wk_signup_error'), true, responses[4]);
                        }
                        if (list_res.indexOf("phone") > 0) {
