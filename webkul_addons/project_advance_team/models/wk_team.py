...
class WkTeam(models.Model):
    ...
    description = fields.Text(string="Description",
                              track_visibility='onchange')
    members = fields.Many2many('res.users', 'team_members', 'team_id',
                               'user_id', 'Members', help="""A group of people with a full set of complementary skills required to complete a job.""", track_visibility='onchange'
                               )
    manager = fields.Many2one(comodel_name="res.users", string='Manager', required=True,
                              default=lambda self: self.env.uid, track_visibility='onchange')
    project_ids = fields.One2many(
        "project.project", 'wk_team_id', string="Projects")
    project_count = fields.Integer(string="count", compute="get_count")
    parent_id = fields.Many2one(
        'wk.team', 'Parent Team', index=True, ondelete='cascade', track_visibility='onchange')
    child_ids = fields.One2many(
        'wk.team', 'parent_id', 'Child Teams', track_visibility='onchange')
    department_id = fields.Many2one(
        'hr.department', string="Department", compute="get_department_id", store=True, track_visibility='onchange')
    ...
    def _recursive_search_of_teams(self):
        ...
        return [team.members.ids + [team.manager.id] for team in self] + members_list
    ...
    def name_get(self):
        ...
        return [(team.id, " / ".join(reversed(get_names(team)))) for team in self]
        ...
        if name:
            ...
            if parents:
                ...
                for i in range(1, len(team_names)):
                    domain = [
                        [('name', operator, ' / '.join(team_names[-1 - i:]))], domain]
                    if operator in expression.NEGATIVE_TERM_OPERATORS:
    ...
    def check_access_rule(self, operation):
        ...
        if self._context.get('params') and action.id == self._context.get('params').get('action'):
