...
class ProjectTaskType(models.Model):
    ...
    def _check_unique_sequence(self):
        """ verifies unique sequence. """
        for stage in self:
            stage_ids = self.search(
                [('sequence', '=', stage.sequence), ('id', '!=', stage.id)])
            if stage_ids:
                raise exceptions.ValidationError(_('Sequence must be Unique.'))
...
class ProjectStageGroup(models.Model):
    ...
    stages_ids = fields.Many2many(
        'project.task.type', 'project_group_type_rel', 'group_id', 'type_id', string='Tasks Stages')
...
class ProjectProject(models.Model):
    ...
    def get_default_stage(self):
        return self.env['project.task.type'].search([('fold', '=', False)]).ids or False

    @api.model
    def get_default_stage_group(self):
        return self.env['project.stage.group'].search([('default', '=', True)], limit=1).id or False

    wk_team_id = fields.Many2one(
        comodel_name="wk.team", string="Team", help="Default team of this project", track_visibility='onchange',)
    wk_extra_team_ids = fields.Many2many("wk.team", "project_teams", "project_id", "team_id",
                                      string="Extra Teams", help="You may add extra team which will also be involved in this project", track_visibility='onchange',)
    members = fields.Many2many('res.users', 'project_members', 'project_id',
                               ...
                               the tasks related to this project.""", track_visibility='onchange',
                               )
    type_ids = fields.Many2many('project.task.type', 'project_task_type_rel',
                                'project_id', 'type_id', string='Tasks Stages', track_visibility='onchange',)
    stage_group_id = fields.Many2one(
        'project.stage.group', string="Stage Group", default=get_default_stage_group, track_visibility='onchange',)
    ...
    def write(self, vals):
        ...
        old_teams = self._get_many2many_track_visibility_names(self.wk_extra_team_ids)
        res = super(ProjectProject, self).write(vals)
        new_members = self._get_many2many_track_visibility_names(self.members)
        new_teams = self._get_many2many_track_visibility_names(self.wk_extra_team_ids)
        if not self.wk_team_id and not self.wk_extra_team_ids and not self.members:
            raise UserError(
                _("Opps!!! you haven't configured any team or extra members for this project. so please do it otherwise you won't be able to assign any task of this project to your team members."))
        if vals.get('stage_group_id'):
            self.type_ids = [(6, 0, self.stage_group_id.stages_ids.ids)]
        if old_members != new_members:
            self.message_post(body="<b>Members:</b> %s &#8594; %s" % (old_members, new_members))
        if old_teams != new_teams:
            self.message_post(body="<b>Extra Teams:</b> %s &#8594; %s" % (old_teams, new_teams))
...
class Task(models.Model):
    ...
    def _onchange_project(self):
        ...
        if self.project_id:
            ...
            self.user_id = False
            return {'domain': {'stage_id': [('id', 'in', self.project_id.type_ids.ids)], 'user_id': [('id', 'in', users)]}}
        else:
            self.user_id = self._uid
            if self.stage_group_id.stages_ids:
                self.stage_id = self.stage_group_id.stages_ids.ids[0]
                return {'domain': {'stage_id': [('id', 'in', self.stage_group_id.stages_ids.ids)], 'user_id': [('id', 'in', users)]}}
    ...
    def onchange_stage_group_id(self):
        if not self.project_id and self.stage_group_id:
            if self.stage_group_id.stages_ids:
                self.stage_id = self.stage_group_id.stages_ids.ids[0]
                return {'domain': {'stage_id': [('id', 'in', self.stage_group_id.stages_ids.ids)]}}
            ...
            if self.project_id.stage_group_id:
                ...
                return {'domain': {'stage_id': [('id', 'in', self.project_id.type_ids.ids)]}}
            else:
                self.stage_id = self.stage_group_id.stages_ids.ids[0]
                return {'domain': {'stage_id': [('id', 'in', self.stage_group_id.stages_ids.ids)]}}
    ...
    def _onchange_user(self):
        res = super(Task, self)._onchange_user()
        users = self.get_user_domin()
        return {'domain': {'user_id': [('id', 'in', users)]}}
        return res
    ...
    stage_group_id = fields.Many2one(
        'project.stage.group', string="Stage Group", default=get_default_stage_group, required=True)
    user_id = fields.Many2one('res.users',
                              string='Assigned to', domain="[('id', 'in', user_ids[0][2])]",
                              default=lambda self: self.env.uid,
                              index=True, track_visibility='always', help="You'll only assign this task to project team and extra members. so if you're not able to see any members then please configure the team and members for particular project.")
    user_ids = fields.Many2many("res.users", 'task_id', 'user_id',
...
class ProjectIssues(models.Model):
    ...
    user_id = fields.Many2one('res.users', string='Assigned to', index=True, track_visibility='onchange', default=lambda self: self.env.uid,
                              domain="[('id', 'in', user_ids[0][2])]", help="You'll only assign this task to project team and extra members. so if you're not able to see any members then please configure the team and members for particular project.")
    user_ids = fields.Many2many("res.users", 'task_id', 'user_id',
                                compute="issue_user_domain", string="Task Users")
