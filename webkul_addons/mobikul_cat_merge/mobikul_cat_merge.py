...
# this field is added to base module of mobikul
...
class mobikul_cat_merge(models.TransientModel):
    ...
    def linked_mobikul_cat_with_products(self,show_wizard=True):
        ...
        for cat_id in website_catg_ids:
            website_prods = self.env['product.template'].search([('public_categ_ids', 'in', cat_id.id)])
    ...
    def export_categories(self,show_wizard=True):
        ...
        if not error_message:
            error_message = "%s New Category(s) has been merged with Mobikul." % (length)
        if length == 0:
    ...
    def sync_mobikul_cat_with_web_cat(self):
        msg = self.export_categories(show_wizard=False)
        ...
        if self.sync_type == "name":
            message = "Done all names are synced with mobikul categories from Website category."
        elif self.sync_type == "name_parent":
            message = "Done all names and parent categories are synced with mobikul categories from Website category."
        elif self.sync_type == "name_parent_seq":
            message = "Done all names, sequences and parent categories are synced with mobikul categories from Website category."
