...
class product_uom_price(models.Model):
    ...
    def create(self, vals):
        ...
        if product_template.uom_id and product_template.uom_id.category_id != unit.category_id:
            raise UserError('Please choice unit the same category of base product unit is %s, for made linked stock inventory' % product_template.uom_id.category_id.name)
        ...
        if vals.get('uom_id', None):
            ...
            for uom_price in self:
                if uom_price.product_tmpl_id.uom_id and uom_price.product_tmpl_id.uom_id.category_id != unit_will_change.category_id:
                    raise UserError(
                        'Please choice unit the same category of base product unit is %s, for made linked stock inventory' % uom_price.product_tmpl_id.uom_id.category_id.name)
